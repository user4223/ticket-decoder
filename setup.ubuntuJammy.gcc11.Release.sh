...
docker build . -t ubuntu-jammy-gcc11-ticket-decoder-builder -f etc/ubuntuJammy.gcc11.Release.Dockerfile
docker run -it \
  ...
  --mount type=bind,source="$(pwd)"/build/ubuntuJammy.gcc11.Release,target=/ticket-decoder/build/Release \
