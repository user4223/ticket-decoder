...
mkdir -p ${WORKSPACE_ROOT}/build/ubuntu22.gcc12.Release

docker buildx build ${WORKSPACE_ROOT} \
  -t ubuntu22-gcc12-ticket-decoder-builder \
...
docker run -it \
  ...
  --mount type=bind,source=${WORKSPACE_ROOT}/build/ubuntu22.gcc12.Release,target=/ticket-decoder/build/Release/bin \
  ubuntu22-gcc12-ticket-decoder-builder
