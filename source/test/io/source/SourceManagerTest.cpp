...
namespace io::api
{
    ...
    {
        auto sourceManager = SourceManager::create(test::support::getLoggerFactory(), LoadResult(std::vector<InputElement>{}));
    ...
    TEST(SourceManager, initial)
    {
        char *first = (char *)"a";
        char *second = (char *)"b";
        auto sourceManager = SourceManager::create(test::support::getLoggerFactory(), LoadResult({InputElement::fromFile("first", cv::Mat{1, 1, CV_8UC1, first}),
