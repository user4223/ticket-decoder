#include <gtest/gtest.h>

#include "lib/utility/include/SignatureChecker.h"
#include "lib/utility/include/Logging.h"
#include "lib/utility/include/Base64.h"

#include <filesystem>

namespace utility
{
  static std::unique_ptr<SignatureChecker> getSignatureChecker()
  {
    auto loggerFactory = LoggerFactory::create();
    return SignatureChecker::create(loggerFactory, std::filesystem::current_path()
                                                       .parent_path()
                                                       .parent_path()
                                                       .parent_path()
                                                       .append("doc")
                                                       .append("UIC_PublicKeys_20221107.xml"));
  }

  TEST(SignatureChecker, certificateNotFound)
  {
    EXPECT_EQ(SignatureChecker::Result::KeyNotFound, getSignatureChecker()->check("4242", "23232323", {}, {}));
  }

  TEST(SignatureChecker, valid_UIC918_9_FV_SuperSparpreis)
  {
    auto const signature = base64::decode("eC4v4YSh2F6J6TOLKY7GGuuiSM5yIFbKlAqWfIodORJuLGKMT86pG6NSFqCjUPiU3l69e4kJkg/elH/u3g4gxA==");
    EXPECT_EQ(64, signature.size());
    auto const message = base64::decode("");
    EXPECT_EQ(199, message.size());
    EXPECT_EQ(SignatureChecker::Result::Successful, getSignatureChecker()->check("1080", "00002", message, signature));
  }

  TEST(SignatureChecker, invalid_UIC918_9_FV_SuperSparpreis)
  {
    auto const signature = base64::decode("eC4v4YSh2F6J6TOLKY7GGuuiSM5yIFbKlAqWfIodORJuLGKMT86pG6NSFqCjUPiU3l69e4kJkg/elH/u3g4gxA==");
    EXPECT_EQ(64, signature.size());
    auto message = base64::decode("");
    message[23] = 42;
    EXPECT_EQ(199, message.size());
    EXPECT_EQ(SignatureChecker::Result::Failed, getSignatureChecker()->check("1080", "00002", message, signature));
  }

  TEST(SignatureChecker, valid_UIC918_3_Quer_durchs_Land_Ticket)
  {
    auto const signature = base64::decode("MCwCFAg/3Fhpe1JJTU2zykgZlmU8rvroAhQWzi/E35MPW079nE+5JctGrcos4wAAAAA=");
    EXPECT_EQ(50, signature.size());
    auto const message = base64::decode("");
    EXPECT_EQ(346, message.size());
    EXPECT_EQ(SignatureChecker::Result::Successful, getSignatureChecker()->check("0080", "00007", message, signature));
  }
}
