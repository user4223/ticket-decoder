...
namespace dip::detection::detail
{
  ...
  static std::vector<double> sideLengths(api::Descriptor::ContourType const &contour, bool sort)
  {
  ...
  Pipe::FilterType Pipe::convexHull()
  {
    return [](auto &&descriptor)
    {
      descriptor.forEachContour([](auto &descriptor)
                                {
                  api::Descriptor::ContourType output;
  ...
  PipeDescriptor handleDebug(PipeDescriptor &&input, unsigned int const debugStep)
  {
    ...
    {
      auto clone = std::vector<api::Descriptor>{input.contours.size()};
