...
namespace utility
{
  ...
  JsonBuilder &JsonBuilder::add(UTF8String_t *const field)
  {
    ...
    return add(StringDecoder::decodeUTF8(std::span<uint8_t const>(field->buf, field->size)));
  }

  template <>
  JsonBuilder &JsonBuilder::add(std::string name, UTF8String_t const &field)
  {
    return add(name, StringDecoder::decodeUTF8(std::span<uint8_t const>(field.buf, field.size)));
  }
