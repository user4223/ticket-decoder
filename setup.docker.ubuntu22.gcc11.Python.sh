...
docker buildx build ${WORKSPACE_ROOT} -t ubuntu22-gcc11-ticket-decoder-python-builder -f etc/docker/ubuntu22.gcc11.Python.Dockerfile  --build-arg="GCC_VERSION=11"
docker run -it \
